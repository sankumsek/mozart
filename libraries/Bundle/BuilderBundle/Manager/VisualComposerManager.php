...
class VisualComposerManager extends AbstractBuilderManager implements EventSubscriberInterface
{
    const VERSION = '4.3.3';
    /**
    ...
    {
        define('WPB_VC_VERSION', static::VERSION);
        /*
        ...
        $this->setPaths(
            array(
                ...
                'APP_DIR' => basename(__DIR__),
                'CONFIG_DIR' => __DIR__.'/../Resources/config',
                'ASSETS_DIR' => __DIR__.'/../../../../public/builder',
                'CORE_DIR' => __DIR__.'/../../../Builder/classes/core',
                ...
                'VENDORS_DIR' => __DIR__.'/../../../Builder/classes/vendors',
            )
        );
        // Load API
        require $this->path('HELPERS_DIR', 'helpers_factory.php');
        require $this->path('HELPERS_DIR', 'helpers.php');
        require $this->path('CORE_DIR', 'interfaces.php');
        require $this->path('CORE_DIR', 'class-wpb-map.php');
        require $this->path('HELPERS_DIR', 'helpers_api.php');
        require $this->path('HELPERS_DIR', 'filters.php');
        require $this->path('PARAMS_DIR', 'params.php');
        require $this->path('SHORTCODES_DIR', 'shortcodes.php');
    }
    ...
    public function pluginsLoaded()
    {
        ...
        load_plugin_textdomain('js_composer', false, 'mozart/translations/builder/');
    }
    ...
    protected function setMode()
    {
        if (is_admin()) {
            ...
            } elseif (vc_action() === 'vc_upgrade' || (vc_get_param('action') === 'update-selected' && vc_get_param(
                        'plugins'
                    ) === $this->pluginName())
            ) {
    ...
    protected function setVersion()
    {
        ...
        if (!is_string($version) || version_compare($version, static::VERSION) !== 0) {
            add_action('vc_after_init', array(vc_settings(), 'rebuild'));
            update_option('vc_version', static::VERSION);
        }
    }
    ...
    {
        return $this->custom_user_templates_dir !== false ? $this->custom_user_templates_dir.'/'.$template : locate_template(
            'vc_templates'.'/'.$template
        );
    }
    ...
    public function updater()
    {
        if (!isset($this->factory['updater'])) {
            ...
            $updater->setUpdateManager(
                new \Vc_Updating_Manager(static::VERSION, $updater->versionUrl(), vc_plugin_name())
            );
    ...
    {
        return preg_replace('/\s/', '%20', plugins_url('mozart/public/builder/'.$file));
    }
    ...
    {
        $events[MozartEvents::BOOT][] = array('onKernelBoot');
