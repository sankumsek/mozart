...
class SystemInfo
{
    ...
    public static function get($args = array())
    {
        ...
        $return = '<textarea readonly="readonly" onclick="this.focus(); this.select()" id="'
            .$args['container_id'].'"'
            .$args['container_class'].' title="To copy the system info, click below and press Ctrl+C (PC) or Cmd+C (Mac).">';
        ...
        $return .= 'Permalink Structure:      '.(get_option('permalink_structure') ? get_option(
                'permalink_structure'
            ) : 'Default')."\n";
        ...
        if (get_option('show_on_front') === 'page') {
            ...
            $return .= 'Page On Front:            '.($front_page_id !== 0 ? get_the_title(
                        $front_page_id
                    ).' (#'.$front_page_id.')' : 'Unset')."\n";
            $return .= 'Page For Posts:           '.($blog_page_id !== 0 ? get_the_title(
                        $blog_page_id
                    ).' (#'.$blog_page_id.')' : 'Unset')."\n";
        }
        ...
        $return .= 'Table Prefix:             '.'Length: '.strlen($wpdb->prefix).'   Status: '.(strlen(
                $wpdb->prefix
            ) > 16 ? 'ERROR: Too long' : 'Acceptable')."\n";
        $return .= 'WP_DEBUG:                 '.(defined(
                'WP_DEBUG'
            ) ? WP_DEBUG ? 'Enabled' : 'Disabled' : 'Not set')."\n";
        ...
        if ($args['show_inactive_plugins'] === true) {
            $return .= "\n".'-- WordPress Inactive Plugins'."\n\n";
        ...
        if (is_multisite()) {
            ...
            foreach ($plugins as $plugin_path) {
                ...
                $plugin = $active_plugins[$plugin_base];
        ...
        $return .= 'Display Errors:           '.(ini_get('display_errors') ? 'On ('.ini_get(
                    'display_errors'
                ).')' : 'N/A')."\n";
        ...
        $return .= "\n".'-- PHP Extensions'."\n\n";
        $return .= 'cURL:                     '.(function_exists(
                'curl_init'
            ) ? 'Supported' : 'Not Supported')."\n";
        $return .= 'fsockopen:                '.(function_exists(
                'fsockopen'
            ) ? 'Supported' : 'Not Supported')."\n";
        $return .= 'SOAP Client:              '.(class_exists(
                'SoapClient'
            ) ? 'Installed' : 'Not Installed')."\n";
        $return .= 'Suhosin:                  '.(extension_loaded(
                'suhosin'
            ) ? 'Installed' : 'Not Installed')."\n";
