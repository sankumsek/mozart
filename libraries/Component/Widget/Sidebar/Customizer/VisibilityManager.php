...
class VisibilityManager
{
    ...
    protected function get_widget_data($instance)
    {
        ...
        if (!in_array(@$data['action'], $valid_action, true)) {
    ...
    public function admin_widget_button($widget, $return, $instance)
    {
        ...
        ?>">
            ...
            <?php if (!isset($_POST[ 'csb-visibility-button' ])) : ?>
                <a href="#" class="button csb-visibility-button"><span class="dashicons dashicons-visibility"></span> <?php _e('Visibility', CSB_LANG);
        ?></a>
            <?php else : ?>
                <script>jQuery(function () { jQuery('.csb-visibility-<?php echo esc_js($widget->id);
        ...
        ?>" />
                ...
                <div class="csb-option-row csb-action">
                    <label for="<?php echo esc_attr($widget->id);
        ?>-action" class="lbl-show-if toggle-action" <?php if (!$action_show) : ?>style="display:none"<?php endif;
        ?>><?php _e('<b>Show</b> widget if:', CSB_LANG);
        ?></label>
                    <label for="<?php echo esc_attr($widget->id);
        ?>-action" class="lbl-hide-if toggle-action" <?php if ($action_show) : ?>style="display:none"<?php endif;
        ?>><?php _e('<b>Hide</b> widget if:', CSB_LANG);
        ?></label>
                    <input type="hidden" id="<?php echo esc_attr($widget->id);
        ?>-action" name="csb_visibility[action]" value="<?php echo esc_attr($data['action']);
        ?>" />
                    ...
                    <ul class="dropdown" style="display:none">
                        ...
                        <li class="add-filter" data-for=".csb-roles" <?php if (!empty($cond['roles'])) : ?>style="display:none"<?php endif;
        ?>>Roles</li>
        ...
        ?>
                        ...
                        <li class="add-filter" data-for=".csb-pagetypes" <?php if (!empty($cond['pagetypes'])) : ?>style="display:none"<?php endif;
        ?>>Special pages</li>
                        <li class="add-filter" data-for=".csb-posttypes" <?php if (!empty($cond['posttypes'])) : ?>style="display:none"<?php endif;
        ?>>For posttype</li>
                        <li class="csb-group">Taxonomy</li>
                        <?php foreach ($tax_list as $tax_item) :
                            $row_id = 'tax-'.$tax_item->name;
        ?>
                            <li class="add-filter" data-for=".csb-<?php echo esc_attr($row_id);
        ...
        ?>>
                    <label for="<?php echo esc_attr($widget->id);
        ?>-date"><span class="csb-and" style="display:none"><?php _e('AND', CSB_LANG);
        ?></span> <?php _e('On these dates', CSB_LANG);
        ?></label>
                    <i class="dashicons dashicons-trash clear-filter show-on-hover action"></i>
                    <input type="text" id="<?php echo esc_attr($widget->id);
        ?>-date" name="<?php echo esc_attr($block_name);
        ?>[date]" value="<?php echo esc_attr(@$cond['date']);
        ?>" />
                </div>
        ...
        ?>>
                    <label for="<?php echo esc_attr($widget->id);
        ?>-roles"><span class="csb-and" style="display:none"><?php _e('AND', CSB_LANG);
        ?></span> <?php _e('User has role', CSB_LANG);
        ?></label>
                    <i class="dashicons dashicons-trash clear-filter show-on-hover action"></i>
                    <select id="<?php echo esc_attr($widget->id);
        ?>-roles" name="<?php echo esc_attr($block_name);
        ?>[roles][]" multiple="multiple">
                        <?php foreach ($role_list as $role => $details) : ?>
                            <?php $name = translate_user_role($details['name']);
        ?>
                            <?php $is_selected = in_array($role, $cond['roles'], true);
        ?>
        ...
        ?>>
                        <label for="<?php echo esc_attr($widget->id);
        ?>-membership"><span class="csb-and" style="display:none"><?php _e('AND', CSB_LANG);
        ?></span> <?php _e('User has Membership Level', CSB_LANG);
        ?></label>
                        <i class="dashicons dashicons-trash clear-filter show-on-hover action"></i>
                        <select id="<?php echo esc_attr($widget->id);
        ?>-membership" name="<?php echo esc_attr($block_name);
        ?>[membership][]" multiple="multiple">
                            <?php foreach ($membership_levels as $level) : ?>
                                <?php $is_selected = in_array($level['id'], $cond['membership'], true);
        ?>
                                <option <?php selected($is_selected);
        ?> value="<?php echo esc_attr($level['id']);
        ?>">
                                    <?php echo esc_html($level['level_title']);
        ?>
                                    <?php if (!$level['level_active']) {
    _e('(inactive)', CSB_LANG);
        ...
        ?>>
                    <label for="<?php echo esc_attr($widget->id);
        ?>-prosite"><span class="csb-and" style="display:none"><?php _e('AND', CSB_LANG);
        ?></span> <?php _e('Pro Sites Level', CSB_LANG);
        ?></label>
                    <i class="dashicons dashicons-trash clear-filter show-on-hover action"></i>
                    <select id="<?php echo esc_attr($widget->id);
        ?>-prosite" name="<?php echo esc_attr($block_name);
        ?>[prosite][]" multiple="multiple">
                        <?php foreach (array() as $level) : ?>
                            <?php $is_selected = in_array($level['id'], $cond['prosite'], true);
        ?>
                            <option <?php selected($is_selected);
        ?> value="<?php echo esc_attr($level['id']);
        ?>">
        ...
        ?>>
                    <label for="<?php echo esc_attr($widget->id);
        ?>-pagetypes"><span class="csb-and" style="display:none"><?php _e('AND', CSB_LANG);
        ?></span> <?php _e('On these special pages', CSB_LANG);
        ?></label>
                    <i class="dashicons dashicons-trash clear-filter show-on-hover action"></i>
                    <select id="<?php echo esc_attr($widget->id);
        ?>-pagetypes" name="<?php echo esc_attr($block_name);
        ?>[pagetypes][]" multiple="multiple">
                        <?php foreach ($pagetype_list as $type => $name) : ?>
                            <?php $is_selected = in_array($type, $cond['pagetypes'], true);
        ?>
                            <option <?php selected($is_selected);
        ...
        ?>>
                    <label for="<?php echo esc_attr($widget->id);
        ?>-posttypes"><span class="csb-and" style="display:none"><?php _e('AND', CSB_LANG);
        ?></span> <?php _e('On any page of these types', CSB_LANG);
        ?></label>
                    <i class="dashicons dashicons-trash clear-filter show-on-hover action"></i>
                    <select class="posttype" id="<?php echo esc_attr($widget->id);
        ?>-posttypes" name="<?php echo esc_attr($block_name);
        ?>[posttypes][]" multiple="multiple">
                        <?php foreach ($type_list as $type_item) : ?>
                            <?php $is_selected = in_array($type_item->name, $cond['posttypes'], true);
        ?>
                            <option <?php selected($is_selected);
        ?> value="<?php echo esc_attr($type_item->name);
        ?>">
        ...
        $is_selected = in_array($type_item->name, $cond['posttypes'], true);
        $posts = get_posts(
                            array(
        ...
        ?>>
                            <label for="<?php echo esc_attr($widget->id);
        ?>-<?php echo esc_attr($row_id);
        ?>">
                                <input type="checkbox" id="<?php echo esc_attr($widget->id);
        ?>-<?php echo esc_attr($row_id);
        ?>" <?php checked(!empty($cond[ $row_id ]));
        ?> data-lbl-all="<?php echo esc_attr($lbl_all);
        ?>" data-lbl-single="<?php echo esc_attr($lbl_single);
        ?>" />
                                <span class="lbl"><?php echo esc_html(empty($cond[ $row_id ]) ? $lbl_all : $lbl_single);
        ?></span>
                            </label>
                            <div class="detail" <?php if (empty($cond[ $row_id ])) : ?>style="display:none"<?php endif;
        ?>>
                                <select name="<?php echo esc_attr($block_name);
        ?>[<?php echo esc_attr($row_id);
        ?>][]" multiple="multiple">
                                    <?php foreach ($posts as $post) : ?>
                                        <?php $is_selected = in_array($post->ID, $cond[ $row_id ], true);
        ?>
                                        <option <?php selected($is_selected);
        ...
        ?>
                ...
                foreach ($tax_list as $tax_item) {
                    ...
                    ?>>
                        <label for="<?php echo esc_attr($widget->id);
                    ?>-<?php echo esc_attr($row_id);
                    ?>"><span class="csb-and" style="display:none"><?php _e('AND', CSB_LANG);
                    ?></span> <?php echo esc_html($tax_item->labels->name);
                    ?></label>
                        <i class="dashicons dashicons-trash clear-filter show-on-hover action"></i>
                        <select id="<?php echo esc_attr($widget->id);
                    ?>-<?php echo esc_attr($row_id);
                    ?>" name="<?php echo esc_attr($block_name);
                    ?>[<?php echo esc_attr($row_id);
                    ?>][]" multiple="multiple">
                            <?php foreach ($tags as $tag) : ?>
                                <?php $is_selected = in_array($tag->term_id, $cond[ $row_id ], true);
                    ?>
                                <option <?php selected($is_selected);
    ...
    public function maybe_display_widget($instance)
    {
        ...
        if ($condition_true && !empty($cond['roles']) && is_array($cond['roles'])) {
            if (!is_user_logged_in()) {
                ...
                foreach ($current_user->roles as $user_role) {
                    if (in_array($user_role, $cond['roles'], true)) {
                        $has_role = true;
                        break;
                    }
                }
        ...
        if ($condition_true && !empty($cond['posttypes'])) {
            ...
            if (!in_array($posttype, $cond['posttypes'], true)) {
                $condition_true = false;
            } else {
                // Filter for SPECIFIC POSTS.
                if (!empty($cond[ 'pt-'.$posttype ])) {
                    if (!in_array(get_the_ID(), $cond[ 'pt-'.$posttype ], true)) {
                        $condition_true = false;
                    }
                }
            }
                ...
                if (isset($cond[ $tax_key ]) && !empty($cond[ $tax_key ])) {
                    ...
                    if ($tax_type && $tax_type === $tax_item->name) {
                        ...
                        foreach ($tax_terms as $slug) {
                            ...
                            if (in_array($term_data->term_id, $cond[ $tax_key ], true)) {
                                $has_term = true;
                            }
                        }
                    } else {
